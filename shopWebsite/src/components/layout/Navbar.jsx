...
function Navbar() {
	...
	return (
		<header className="w-full h-20 flex items-center justify-between py-0 px-2" style={{
			...
			<nav>
				<ul className='list-none m-0 p-0 flex items-baseline text-xl'>
					...
					<li className='ml-10 hover:text-blue-500 active:text-blue-500'>
						<Link to="/cart">
							My Cart 
							{itemCartCtx.totalOnCart > 0 && (<span className="bg-blue-500 text-blue-50 rounded-md px-2 ml-2">{itemCartCtx.totalOnCart}</span>)}
						</Link>
					</li>
				</ul>
			</nav>
